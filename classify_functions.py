...
def classify(permutation=0):
    ...
    for i in range(np.alen(modalities_to_include)):
        ...
        # The following two line will allow all the variables of the modality
        '''
        ...
        # import pdb; pdb.set_trace()
        # This will allow the "how_many" indices with the highest t-values to be included
        
        tvals,pvals=stats.ttest_ind(X_train1[:,inds],X_train2[:,inds])
        how_many=80
        which_inds=np.abs(tvals).argsort()[-how_many:][::-1]
        index_variables=np.append(index_variables,inds[which_inds]) 
        variable_modality=np.append(variable_modality,np.ones(how_many)*(modalities_to_include[i]))
    ...
    model[0].weight.data=model[0].weight.data.mul_(WW)
    ...
    output_name="result_p"+str(permutation)+"_clinical_only"
    ID=np.random.randint(1000000)
    result_filename="./results/"+output_name+"_"+str(ID)+'.mat'
    #print("Yparxei to arxeio? "+str(os.path.exists(result_filename)))
    if os.path.exists(result_filename)==True:
        ID=ID+1
        result_filename="./results/"+output_name+"_"+str(ID)+'.mat'
...
def classify_ISC(permutation=0):
    #select file to analyse
    print("Is permutation: " + str(permutation))
    ...
    modalities_to_include=np.array([0,1]);
    #modalities_to_include=np.array([0]);
    print("Including modalities:")
    print(modalities_to_include)
    if permutation==1:
    ...
    accuracy=np.sum(target.T==predicted)/len(target)*100
    print("Accuracy",accuracy,"%")
    output_name="result_p"+str(permutation)+"_clinical_ISC"
    ID=np.random.randint(1000000)
    result_filename="./results/"+output_name+"_"+str(ID)+'.mat'
    #print("Yparxei to arxeio? "+str(os.path.exists(result_filename)))
    if os.path.exists(result_filename)==True:
